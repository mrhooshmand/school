...
// import interfaces
import { Category_ } from '../interfaces/categoryInterface'
//import components
import CategoryRow from '../components/table/rows/CategoryRow'

function CategoriesListPage() {

    const [showModal, setShowModal] = useState<boolean>(false)
    const [page, setPage] = useState<number>(0)
    const [categories, setCategories] = useState<Category_[]>()
    ...
    const splitFive = []
      const sliceIntoChunks = (data: Category_[]) => {
          for (let i = 0;i < data.length;i += 5) {
              const chunk = data.slice(i, i + 5)
              splitFive.push(chunk)
          }
      }
    categories && sliceIntoChunks(categories)
    ...
    const getCategory = (category: Category_, action: string) => {
        action === 'edit' ?
    ...
    return (
        <>
            { categories &&
                <div className=" bg-white rounded-lg shadow-xCategory_0 h-4/5 flex flex-col p-10 justify-between mx-5 mt-10">
                    <div>
                        <span className="flex justify-between mb-10">
                        <button className=" text-white rounded px-10 py-3 focus:outline-none shadow-xl shadow-sky-200 bg-[#009DF8]">دسته بندی جدید</button>
                        ...
                        <tbody>
                            { categories.map( (category: Category_) => <CategoryRow key={category.id} category={category} changeModalStatus={changeModalStatus} getCategory={getCategory} /> ) }
                        </tbody>
                        </table>
                    </div>
